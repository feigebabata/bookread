...
public class UGUIManager : MonoSingleton<UGUIManager>
{
	...
	public void CloseCurUI()
	{
		if(m_uiStack.Count>0)
		{
			...
			log("[UGUIManager.CloseCurUI] {0}",oldUI);
			unfocus(oldUI);
			hide(oldUI);
			destroy(oldUI);
			if(m_uiStack.Count>0)
	...
	public void Open<T>(OpenMode _mode=OpenMode.Open,object _data=null) where T : UIBase
	{
		...
		log("[UGUIManager.Open] {0}",newUIType);
		if(!m_panelParent)
		{
	...
	{
		string path = "UIPanel/"+_path;
		Debug.Log(path);
		return Instantiate(Resources.Load<GameObject>(path),_parent);
	}

	void log(string _text,params object[] _val)
	{
